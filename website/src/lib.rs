...
impl Component<WebsiteGlobalState> for Website {
    ...
    fn view(state: &Self, global_state: &WebsiteGlobalState, props: &Self::Props, children: Vec<ComponentSpecification>) -> ComponentSpecification {
        ...
        match global_state.route.as_str() {
            "/examples" => {
                wrapper.push(Examples::component().key("examples")).component()
            }
            "/about" => {
                wrapper.push(About::component().key("about")).component()
            }
            "/" | _ => {
                wrapper
                    .push(index_page().key("index"))
                    .component()
            }
        }
    ...
    fn update(_state: &mut Self, global_state: &mut WebsiteGlobalState, _props: &Self::Props, _message: Event) -> UpdateResult {
