...
pub(crate) fn draw_path(
    ...
    if let Some(line_width) = line_width {
        scene.stroke(&Stroke::new(line_width.0), *affine, &brush, None, &bezier_path);
    } else {
        scene.fill(Fill::EvenOdd, *affine, &brush, None, &bezier_path);
    }
}
            ...
            for command in &tiny_vg.draw_commands {
                match command {
                    ...
                    DrawCommand::OutlineFillRectangles(data) => {
                        let fill_brush =
                            TinyVgHelpers::get_brush(&data.fill_style, &tiny_vg.color_table, override_color);
                        let line_brush =
                            TinyVgHelpers::get_brush(&data.line_style, &tiny_vg.color_table, override_color);
                        for rectangle in &data.rectangles {
                            let rectangle =
                                kurbo::Rect::new(rectangle.x.0, rectangle.y.0, rectangle.height.0, rectangle.height.0);
