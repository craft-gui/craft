...
pub use icu::calendar::types::{DateDuration, Month, Weekday, YearInfo, EraYear};
...
pub fn first_day_of_week() -> Weekday {
    let first_day = cfg_select! {
        windows => windows::first_day_of_week(),
        target_vendor = "apple" => apple::first_day_of_week(),
        _ => None,
    };
    first_day.unwrap_or_else(|| {
        let local = Locale::try_from_str(&sys_locale::get_locale_or_default()).unwrap();
        match WeekPreferences::from(&local).first_weekday {
            None => Weekday::Monday,
            Some(day) => match day {
...
pub fn day_abbreviation(day: Weekday) -> String {
    ...
    day_abbreviation.unwrap_or_else(|| {
        let locale = Locale::try_from_str(&sys_locale::get_locale_or_default()).unwrap();
        let formatter =
...
pub fn current_calendar_start(year: i32, month: Month) -> Date<Gregorian> {
    let mut start = Date::try_new_gregorian(year, month.number(), 1).expect("Invalid date provided by system");

    let start_of_week = first_day_of_week() as i32;
...
pub fn format_date_day_number(date: &Date<Gregorian>) -> String {
    ...
    day_number.unwrap_or_else(|| {
        let locale = Locale::try_from_str(&sys_locale::get_locale_or_default()).expect("Failed to parse system locale");
...
pub fn month_name(month: Month, year: i32) -> String {
...
pub fn year_name(year: i32) -> String {
    ...
    year_name.unwrap_or_else(|| {
        let locale = Locale::try_from_str(&sys_locale::get_locale_or_default())
            .expect("Failed to parse system locale");

        let date = Date::try_new_gregorian(year, 1, 1)
