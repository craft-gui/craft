...
pub(crate) type CraftAccessTree = issho::AccessTree<Arc<Window>>;

thread_local! {
    static ACCESS_TREE: CraftAccessTree = {
        let tree = CraftAccessTree::new();
        tree.set_framework_name("Craft");
        tree.set_native_platform();
        tree
    };
}

pub(crate) fn access_tree() -> CraftAccessTree {
    ACCESS_TREE.with(Clone::clone)
}

pub(crate) fn set_subtree_context(
    element: &mut dyn ElementInternals,
    tree: &CraftAccessTree,
