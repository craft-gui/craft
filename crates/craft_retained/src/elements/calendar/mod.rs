...
use craft_calendar::{DateAddOptions, DateDuration, Month, Weekday, current_calendar_start, current_month, day_abbreviation, first_day_of_week, format_date_day_number, month_name, year_name};
...
use crate::style::{AlignItems, Display, FlexDirection, JustifyContent, Overflow};
...
impl Calendar {
    pub fn new() -> Self {
        let first_day = first_day_of_week();
        ...
        for _ in 0..7 {
            let day = day_abbreviation(current_header_day);
        ...
        let now = Local::now();
        let mut start_date = current_calendar_start(now.year(), Month::new(now.month() as u8));
        for _ in 0..6 {
            let mut week = Container::new()
                .display(Display::Flex)
                .flex_direction(FlexDirection::Row);
            for _ in 0..7 {
                let day_text = Text::new("");
...
impl CalendarInner {
    fn update_calendar(&mut self) {
        let mut start_date = current_calendar_start(self.focus_year, Month::new(self.focus_month));
        for day_element in &self.days {
            let is_in_current_month = start_date.month().ordinal == self.focus_month;
            let date_str = format_date_day_number(&start_date);
            day_element
                .clone()
    ...
    fn setup_years(&mut self) {
        ...
        for year in (self.start_year..(self.end_year + 1)).rev() {
            dropdown.clone().push(Text::new(&year_name(year))).font_size(20.0);
    ...
    fn setup_months(&mut self) {
        ...
        for month in 0..12 {
            dropdown
                .clone()
                .push(Text::new(&month_name(Month::new(month), self.focus_year)))
                .font_size(20.0);
